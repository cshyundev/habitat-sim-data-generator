...
class Camera:
    ...
    def _warp(
        ...
        # Bilinear sampling via scipy (replaces cv2.remap to avoid an OpenCV dep).
        # map_coordinates indexes as [row, col] == [v, u].
        if image.ndim == 3:
            output_image = np.zeros(
                (self.height, self.width, image.shape[2]), dtype=image.dtype
            )
            for c in range(image.shape[2]):
                output_image[..., c] = map_coordinates(
                    image[..., c], [v, u], order=1, mode="constant"
                )
        else:
            output_image = map_coordinates(image, [v, u], order=1, mode="constant")
        if valid_mask:
